...
def _check_plotting_libs() -> bool:
    ...
    if not MATPLOTLIB_AVAILABLE:
        logger.error("Matplotlib is required for plotting. Please install it (`pip install matplotlib seaborn`).")
        libs_ok = False
...
def _create_plot_dir_from_base(base_save_path: Optional[Path]) -> Optional[Path]:
    """
    Creates an output directory for plots. If base_save_path is a file,
    it creates a dir like 'filename_plots'. If it's a dir, it uses it directly.
    Returns None if base_save_path is None or dir creation fails.
    """
    if not base_save_path:
        return None
    try:
        base_path = Path(base_save_path)
        if base_path.is_file(): # If it's a results JSON file path
            plot_dir = base_path.parent / f"{base_path.stem}_plots"
        else: # Assume it's already a directory path for plots for this run
            plot_dir = base_path
        plot_dir.mkdir(parents=True, exist_ok=True)
        return plot_dir
    except Exception as e:
        logger.error(f"Failed to create plot directory using base {base_save_path}: {e}")
        return None
...
class ResultsPlotter:
    ...
    def _plot_learning_curves(history: List[Dict[str, Any]], title: str,
        ...
        if standalone_plot and 'fig' in locals() and fig is not None:
            plt.close(fig)
            return False
    ...
    @staticmethod
    def plot_single_train_results(results_data: Dict[str, Any],
                                  plot_save_dir_base: Optional[Path], # Base path for creating plot dir
                                  show_plots: bool = False):
        """Plots results from a 'single_train' results dictionary."""
        if not results_data or not _check_plotting_libs(): return

        plot_dir = _create_plot_dir_from_base(plot_save_dir_base) if plot_save_dir_base else None
        # If plot_dir is None here, plots will only be shown if show_plots is True, not saved.
        # Helpers need to handle output_path being None.

        run_id = results_data.get('run_id', "unknown_run") # Get run_id from data
        try:
            ...
            if history:
                ...
                ResultsPlotter._plot_learning_curves(
                    history, f"Single Train Learning Curves ({run_id})",
                    output_path=output_file, # _plot_learning_curves needs to handle None output_path
        ...
        except Exception as e: logger.error(f"Failed to plot single_train results for {run_id}: {e}", exc_info=True)


    @staticmethod
    def plot_single_eval_results(results_data: Dict[str, Any],
                                 class_names: List[str],
                                 plot_save_dir_base: Optional[Path],
                                 show_plots: bool = False):
        """Plots results from a 'single_eval' results dictionary."""
        if not results_data or not _check_plotting_libs(): return
        ...
        try:
            ...
            if TABULATE_AVAILABLE:
                ResultsPlotter._generate_metrics_table(
                    results_data,
            ...
            if isinstance(macro_metrics, dict):
                ResultsPlotter._plot_macro_roc_point(
                    macro_metrics,
                    f"{title_prefix}\nMacro Avg ROC Point",
            ...
            if not detailed_data:
                logger.warning(f"Skipping detailed plots for '{title_prefix}': 'detailed_data' missing."); logger.info(f"Finished plotting basic info for single_eval: {run_id}"); return
            ...
            if y_true and y_pred and class_names and SKLEARN_METRICS_AVAILABLE:
                 ResultsPlotter._plot_confusion_matrix(y_true, y_pred, class_names, f"{title_prefix}\nConfusion Matrix", plot_dir / "confusion_matrix.png" if plot_dir else None, show_plots)

            roc_curve_points = detailed_data.get('roc_curve_points')
            if roc_curve_points: ResultsPlotter._plot_roc_curves(roc_curve_points, title_prefix, plot_dir / "roc_curves.png" if plot_dir else None, show_plots)
            pr_curve_points = detailed_data.get('pr_curve_points')
            if pr_curve_points: ResultsPlotter._plot_pr_curves(pr_curve_points, title_prefix, plot_dir / "pr_curves.png" if plot_dir else None, show_plots)

            logger.info(f"Finished plotting for single_eval: {run_id}")
        except Exception as e: logger.error(f"Failed to plot single_eval results for {run_id}: {e}", exc_info=True)

    @staticmethod
    def plot_non_nested_cv_results(results_data: Dict[str, Any],
                                   plot_save_dir_base: Optional[Path],
                                   show_plots: bool = False):
        ...
        try:
            ...
            if cv_results and TABULATE_AVAILABLE:
                ResultsPlotter._generate_grid_search_table(
                    cv_results,
        ...
        except Exception as e: logger.error(f"Failed to plot non_nested_cv results for {run_id}: {e}", exc_info=True)

    @staticmethod
    def plot_nested_cv_results(results_data: Dict[str, Any],
                               plot_save_dir_base: Optional[Path],
                               show_plots: bool = False):
        ...
        try:
            ...
            if outer_scores and isinstance(outer_scores, dict) and sns:
                 ...
                 valid_metrics_data = {m: v[~np.isnan(v)] for m, v in valid_metrics_data.items() if len(v[~np.isnan(v)]) > 0}
                 ...
                 if valid_metrics_to_plot:
                     ...
                     for ax, metric in zip(axes, valid_metrics_to_plot):
                     ...
                     # Save only if plot_dir exists
                     output_path_scores = plot_dir / "outer_fold_score_distributions.png" if plot_dir else None
                     _save_show_plot(fig, output_path_scores, show_plots)
            ...
            inner_scores_per_fold = results_data.get('outer_fold_inner_cv_best_score')
            if best_params_per_fold and inner_scores_per_fold and TABULATE_AVAILABLE:
                ResultsPlotter._generate_nested_cv_param_table(
                    best_params_list=best_params_per_fold,
        ...
        except Exception as e: logger.error(f"Failed to plot nested_cv results for {run_id}: {e}", exc_info=True)

    @staticmethod
    def plot_cv_model_evaluation_results(results_data: Dict[str, Any],
                                         plot_save_dir_base: Optional[Path],
                                         class_names: List[str],
                                         show_plots: bool = False):
        ...
        try:
            ...
            if aggregated_metrics:
                ResultsPlotter._plot_cv_aggregated_metrics(
                    ...
                    plot_dir / "aggregated_metrics_ci.png" if plot_dir else None,
                    show_plots
                )
            ...
            if fold_scores and isinstance(fold_scores, dict) and sns:
                 ...
                 if valid_metrics_to_plot:
                     ...
                     fig.suptitle(f"{title_prefix} - Fold Score Distributions", fontsize=16)
                     plot_idx = 0
                     ...
                     # Save only if plot_dir exists
                     output_path_scores = plot_dir / "fold_score_distributions.png" if plot_dir else None
                     _save_show_plot(fig, output_path_scores, show_plots)
            ...
            if not fold_details or not isinstance(fold_details, list):
                ...
                if fold_histories and isinstance(fold_histories, list) and len(fold_histories) == n_folds_actual and any(h for h in fold_histories if h):
                    ...
                    for fold_idx, history in enumerate(fold_histories):
                        ...
                        if history:
                            ...
                            plot_success = ResultsPlotter._plot_learning_curves(
                                history, f"Fold {fold_idx+1}",
                                plot_dir / "dummy.png" if plot_dir else None,
                                False, ax_loss=ax_loss, ax_acc=ax_acc)
                    ...
                    if plotted_any_lc:
                        ...
                        # Save only if plot_dir exists
                        output_path_curves = plot_dir / "learning_curves_all_folds.png" if plot_dir else None
                        _save_show_plot(fig, output_path_curves, show_plots)
                ...
                if SKLEARN_METRICS_AVAILABLE:
                    ...
                    for fold_idx, fold_data in enumerate(fold_details):
                        ...
                        if det_data_cm and isinstance(det_data_cm, dict):
                            ...
                            if y_true and y_pred and class_names:
                                plot_success_cm = ResultsPlotter._plot_confusion_matrix(
                                    y_true, y_pred, class_names, f"Fold {fold_idx+1}",
                                    plot_dir / "dummy.png" if plot_dir else None, False, ax=ax)
                    ...
                    if any(ax.has_data() for ax in axes_cm_flat[:plot_idx_cm]): # Check if anything was actually plotted
                        # Save only if plot_dir exists
                        output_path_matrices = plot_dir / "confusion_matrices_all_folds.png" if plot_dir else None
                        _save_show_plot(fig, output_path_matrices, show_plots)
                ...
                for fold_idx, fold_data in enumerate(fold_details):
                     ...
                     if roc_curve_points_fold and isinstance(roc_curve_points_fold, dict):
                         plot_success_roc = ResultsPlotter._plot_roc_curves(
                             roc_curve_points_fold, f"Fold {fold_idx+1}",
                             plot_dir / "dummy.png" if plot_dir else None,
                             False, ax=ax)
                     if not plot_success_roc:
                ...
                if any(ax.has_data() for ax in axes_roc_flat[:plot_idx_roc]):
                    # Save only if plot_dir exists
                    output_path_curves = plot_dir / "roc_curves_all_folds.png" if plot_dir else None
                    _save_show_plot(fig, output_path_curves, show_plots)
                ...
                for fold_idx, fold_data in enumerate(fold_details):
                    ...
                    if pr_curve_points_fold and isinstance(pr_curve_points_fold, dict):
                        plot_success_pr = ResultsPlotter._plot_pr_curves(
                            pr_curve_points_fold, f"Fold {fold_idx+1}",
                            plot_dir / "dummy.png" if plot_dir else None,
                            False, ax=ax)
                ...
                if any(ax.has_data() for ax in axes_pr_flat[:plot_idx_pr]):
                    # Save only if plot_dir exists
                    output_path_curves = plot_dir / "pr_curves_all_folds.png" if plot_dir else None
                    _save_show_plot(fig, output_path_curves, show_plots)
                else: plt.close(fig_pr)

                # --- Plot CV Macro Average Points (across all folds) ---
                ResultsPlotter._plot_cv_macro_roc_points(
                    fold_details, f"{title_prefix}\nMacro Average ROC Points per Fold",
                    plot_dir / "cv_macro_roc_points.png" if plot_dir else None, show_plots)
            ...
            if fold_details and isinstance(fold_details, list) and len(fold_details) > 0 and fold_details[0] and TABULATE_AVAILABLE:
                 ...
                 ResultsPlotter._generate_metrics_table(
                     fold_details[0],
                     output_path=plot_dir / "fold_1_metrics_table.txt" if plot_dir else None)

            logger.info(f"Finished plotting for cv_model_evaluation: {run_id}")

        except Exception as e: logger.error(f"Failed to plot cv_model_evaluation results for {run_id}: {e}", exc_info=True)

    @staticmethod
    def plot_predictions(predictions_output: List[Dict[str, Any]],  # Takes raw prediction list
                         plot_save_dir: Optional[Path],  # Directory to save, not a base path
                         show_plots: bool = False,
                         max_cols: int = 4):  # <<< NEW PARAMETER
        """
        Plots a grid of images with their top predicted class and confidence.

        Args:
            predictions_output: List of prediction result dictionaries.
                         Each dict should have 'image_path', 'predicted_class_name', 'confidence'.
            plot_save_dir: Directory to save the plot.
            show_plots: Whether to display the plot interactively.
            max_cols (int): Maximum number of images to display per row in the grid.
        """
        if not predictions_output or not MATPLOTLIB_AVAILABLE: logger.warning("No predictions to plot or matplotlib not available."); return
        if not _check_plotting_libs(): return

        # plot_save_dir is already the specific directory for this plot run's outputs
        # _create_output_dir is not needed here as the dir is passed directly
        # However, it might be None if only showing.

        logger.info(f"Plotting {len(predictions_output)} predictions with max {max_cols} columns...")

        num_images = len(predictions_output)
        if num_images == 0:
            return

        # Determine grid size based on max_cols
        if max_cols <= 0:  # Ensure max_cols is positive
            max_cols = 4
            logger.warning(f"max_cols must be positive, defaulting to {max_cols}.")

        cols = min(num_images, max_cols)  # Actual number of columns
        rows = (num_images + cols - 1) // cols  # Calculate rows needed

        # Adjust figure size dynamically
        # These are heuristics, you might need to fine-tune them
        base_img_width_inch = 3.5  # Assumed typical width for one image subplot
        base_img_height_inch = 4.0  # Assumed typical height for one image subplot (allows for title)
        fig_width = cols * base_img_width_inch
        fig_height = rows * base_img_height_inch

        # Add a cap to prevent excessively large figures if there are many rows
        max_fig_height = 30  # inches
        ...
        for i, pred_data in enumerate(predictions_output):
            if i >= len(axes_flat):  # Should not happen if rows/cols calculated correctly
                logger.warning(
                    f"Attempted to plot more images than available subplot slots. Max slots: {len(axes_flat)}, Image index: {i}")
                break

            ax = axes_flat[i]
            try:
                img_path_str = pred_data.get('image_path', 'Unknown')
                img_path = Path(img_path_str)

                if not img_path.exists() or not img_path.is_file():
                    logger.warning(f"Image not found for plotting: {img_path}")
                    ax.text(0.5, 0.5, "Image\nNot Found", ha="center", va="center", transform=ax.transAxes, fontsize=8,
                            color='gray')
                    ax.set_title(f"{img_path.name}", fontsize=7, color='gray', y=0.95)  # Slightly lower title
                    ax.set_xticks([])
                    ax.set_yticks([])
                ...
                # Load image using matplotlib's imread which uses Pillow backend
                img = plt.imread(img_path)
                ax.imshow(img)
                plotted_images_count += 1

                title_str = f"Pred: {pred_data['predicted_class_name']}\nConf: {pred_data['confidence']:.2f}"
                top_k = pred_data.get('top_k_predictions')
                if top_k and isinstance(top_k, list) and len(top_k) > 1:
                    # Add second prediction if different from top
                ...
                logger.error(f"Error plotting image {pred_data.get('image_path', 'Unknown')}: {e}")
                ax.text(0.5, 0.5, "Error Plotting", ha="center", va="center", transform=ax.transAxes, fontsize=8,
                        color='red')
                ax.set_title(f"{Path(pred_data.get('image_path', 'Error')).name}", fontsize=7, color='gray', y=0.95)
                ax.axis('off')

        # Hide unused subplots
        # Start hiding from the index where the last image was plotted (or attempted)
        for j in range(i + 1, len(axes_flat)):
            axes_flat[j].set_visible(False)

        # If no images were actually plotted successfully, don't save an empty figure
        if plotted_images_count == 0:
            logger.warning("No images were successfully plotted for the prediction grid.")
            if 'fig' in locals():  # Ensure fig was created
                plt.close(fig)
            return

        # Adjust layout to prevent titles from overlapping
        # This might need to be called before saving if titles are long or many rows
        try:
            fig.tight_layout(pad=1.0, h_pad=2.5, w_pad=1.5)  # More padding

            if plot_save_dir:  # Only save if plot_dir is provided
                output_file = plot_save_dir / "image_predictions_grid.png" if plot_save_dir else None
                _save_show_plot(fig, output_file, show_plots)  # _save_show_plot needs to handle output_file=None
            elif show_plots:  # If no plot_dir but show_plots is True
                plt.show()
                plt.close(fig)
            else:  # No plot_dir and not showing, just close
                plt.close(fig)
        except ValueError:  # Can happen if axes are invisible etc.
