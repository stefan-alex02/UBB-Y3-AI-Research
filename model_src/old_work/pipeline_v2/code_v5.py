...
class TransformedSubset(Dataset):
    ...
    def __init__(self, dataset: Dataset, indices: List[int], transform: Optional[Callable]):
        ...
        Args:
            ...
            transform (Optional[Callable]): The transformation pipeline_v1 to apply to images
...
class ClassificationPipeline:
    ...
    def __init__(self,
                 ...
                 patience: int = 10):
        """
        Initializes the classification pipeline_v1.
    ...
    def single_train(self,
        ...
        # Clone adapter to avoid modifying the pipeline_v1's main one if single_train is intermediate
        ...
        # Update the main pipeline_v1 adapter with the trained one if desired (or return it)
    ...
    def load_model(self, model_path: Union[str, Path]) -> None:
        """
        Loads a trained model's state_dict from a file into the pipeline_v1's model adapter.
...
class PipelineExecutor:
    """
    Executes a sequence of classification pipeline_v1 methods (train, eval, search)
    defined by the user.
    ...
    Attributes:
        pipeline (ClassificationPipeline): The underlying classification pipeline_v1 instance.
    ...
    def __init__(self,
                 ...
                 # Pass pipeline_v1 config params here
                 img_size: Tuple[int, int] = (224, 224),
                 val_split_ratio: float = 0.2,
                 data_augmentation: bool = True,
                 lr: float = 0.001,
        ...
        Args:
            ...
            methods (List[Tuple[str, Dict[str, Any]]]): A list of tuples, where each tuple contains
                                                        the name of the pipeline_v1 method to run (str)
                                                        and a dictionary of its parameters (Dict).
                                                        Example: [('single_train', {'max_epochs': 50}), ('single_eval', {})]
            img_size (Tuple[int, int]): Image size for the pipeline_v1.
            val_split_ratio (float): Validation split ratio for the pipeline_v1.
            data_augmentation (bool): Data augmentation flag for the pipeline_v1.
            lr (float): Default learning rate for the pipeline_v1's model adapter.
            max_epochs (int): Default max epochs for the pipeline_v1's model adapter.
            batch_size (int): Default batch size for the pipeline_v1's model adapter.
            patience (int): Default early stopping patience for the pipeline_v1.
        ...
        # Initialize the underlying pipeline_v1
    ...
    def run(self) -> Dict[str, Any]:
        ...
        for i, (method_name, params) in enumerate(self.methods_to_run):
            ...
            try:
                # Get the corresponding method from the pipeline_v1 instance
