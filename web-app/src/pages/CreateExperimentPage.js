...
import ParamsInfoModal from '../components/Modals/ParamsInfoModal';
    ...
    methodsSequence: Yup.array()
        .of(
            Yup.object().shape({
                ...
                cv: Yup.number().integer().min(2).optional().nullable(),
...
const jsonStringToKvArray = (jsonString) => {
    try {
        ...
        if (typeof obj !== 'object' || obj === null) return [{ keyName: '', keyValue: '' }];
                    ...
                    <FormControl fullWidth required error={methodTouched?.method_name && Boolean(methodErrors?.method_name)} sx={{mb:1.5}}>
                        ...
                        <Field
                            ...
                            onChange={(e) => {
                                ...
                                handleChange(e);
                                const defaults = METHOD_DEFAULTS[newMethodName] || METHOD_DEFAULTS.single_train;
                                ...
                                allOptionalKeys.forEach(key => {
                                    setFieldValue(`methodsSequence[${index}].${key}`, defaults[key] !== undefined ? defaults[key] : (key.endsWith('_level') ? 1 : undefined));
                                });
                            }}
                        >
                        ...
                        <Box sx={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1}}>
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                <Tooltip title="Parameter Info/Help">
                                    <IconButton size="medium" onClick={() => setStepInfoModalOpen(true)} sx={{ mr: 0.5, p: '6px' }}>
                                ...
                                >
                                    <ToggleButton value="kv" aria-label="key-value editor" sx={{lineHeight: 1 }}>
                                        <Tooltip title="Key-Value Editor"><ListAltIcon fontSize="medium"/></Tooltip>
                                    </ToggleButton>
                                    <ToggleButton value="json" aria-label="raw json editor" sx={{lineHeight: 1}}>
                                        <Tooltip title="Raw JSON Editor"><CodeIcon fontSize="medium"/></Tooltip>
                                    </ToggleButton>
                                </ToggleButtonGroup>
                            </Box>
                        </Box>
                        ...
                        {method.paramsEditorMode === 'json' ? (
                            ...
                            <FieldArray name={`methodsSequence[${index}].params`}>
                                {({ push: kvPush, remove: kvRemove }) => (
                                    <Box sx={{opacity: 1, pointerEvents: 'auto' }}>
                                        {Array.isArray(method.params) && method.params.map((kvPair, kvIndex) => (
                                            <Grid container spacing={1} key={kvIndex} alignItems="center" sx={{ mb: 2, mt: kvIndex === 0 ? 1.5 : 0 }}>
                                                <Grid item xs={5}>
                                                    <Field
                                                        as={TextField}
...
const CreateExperimentPage = () => {
    ...
    const initialValues = {
        ...
        test_split_ratio_if_flat: 0.2,
        force_flat_for_fixed_cv: false,
    };
                    ...
                    <Formik
                        ...
                        onSubmit={async (values, { setSubmitting, resetForm }) => {
                            ...
                            const javaMethodsSequence = values.methodsSequence.map((uiMethod, index) => {
                                ...
                                } else if (typeof uiMethod.params === 'string') {
                                    try {
                                ...
                                if (uiMethod.method_name === 'non_nested_grid_search' || uiMethod.method_name === 'nested_grid_search') {
                                    methodPayloadForJava.param_grid = processedParamsObject;
                                    methodPayloadForJava.params = undefined;
                                } else if (uiMethod.method_name === 'single_train' || uiMethod.method_name === 'cv_model_evaluation') {
                                    methodPayloadForJava.params = processedParamsObject;
                                    methodPayloadForJava.param_grid = undefined;
                                }
                                ...
                                if (uiMethod.method_search_type === 'random') {
                                    ...
                                    methodPayloadForJava.n_iter = undefined;
                                }
                                ...
                                <Grid container spacing={3}>
                                    ...
                                    <Grid item xs={12} sx={{ mt: 3 }}>
                                        ...
                                        <FieldArray name="methodsSequence">
                                            {({ push, remove }) => (
                                                <Grid item xs={12}>
                                                    {values.methodsSequence.map((methodItem, idx) => (
                                                        <MethodStepCard
                                                    ...
                                                    <Button type="button" startIcon={<AddIcon />} onClick={() => {
                                                        const defaultNewMethod = METHOD_DEFAULTS.single_train;
                                                        push({
                                                            ...defaultNewMethod,
                                                            paramsEditorMode: 'json',
